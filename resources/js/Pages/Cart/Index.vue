...
const eliminarDelCarrito = (item) => {
    ...
    if (selectedItem.value && selectedItem.value.articulo.id === item.articulo.id) {
        selectedItem.value = localCartItems.value.length > 0 ? localCartItems.value[0] : null;
    }
...
// Obtener el precio aplicando el descuento si existe
const obtenerPrecioFinal = (articulo) => {
    return calcularPrecioConDescuento(articulo.precio, articulo.descuento);
};

// Preparar las imágenes para el carousel
const getImagenesToCarousel = (articulo) => {
    if (!articulo || !articulo.imagenes || articulo.imagenes.length === 0) {
        return [{ src: '/img/placeholder.webp', alt: 'Imagen no disponible' }];
    }

    return articulo.imagenes.map(imagen => {
        const ruta = isFullUrl(imagen.ruta) ? imagen.ruta : `/storage/${imagen.ruta}`;
        return {
            src: ruta,
            alt: imagen.alt || `Imagen de ${articulo.nombre}`
        };
    });
};
...
<template>
    <div class="bg-gray-50 text-gray-800 dark:bg-black dark:text-white min-h-screen">
        <div class="flex flex-col items-start justify-center selection:bg-purple-500 selection:text-white dark:selection:bg-[#FF2D20]">
            <Navbar :cart-items-count="cartItemCount" />

            <div class="px-8 flex flex-row min-w-full border-b border-gray-300 dark:border-white/20 bg-gradient-to-r from-purple-100 to-transparent dark:from-purple-900/10">
            ...
            <div class="flex flex-col md:flex-row w-full min-h-screen">
                ...
                <div class="w-full md:w-2/5 p-4 md:p-6 border-r border-gray-300 dark:border-white/20 bg-gradient-to-b from-purple-50 to-transparent dark:from-purple-900/5">
                    <div v-if="localCartItems.length > 0" class="space-y-4">
                        ...
                        <div
                            ...
                            :class="selectedItem && selectedItem.articulo.id === item.articulo.id ? 'border-2 border-purple-500 scale-[1.02] shadow-md bg-purple-50 dark:bg-gray-800/60' : 'hover:border-purple-200 hover:bg-purple-50 dark:hover:bg-gray-800/50'"
                        >
                            <div class="flex-shrink-0 flex flex-col items-center justify-center ml-1">
                                <div class="relative w-14 h-14 overflow-hidden rounded-lg shadow-inner bg-gray-100 dark:bg-gray-700">
                            ...
                            <div class="ml-4 flex-grow">
                                <div class="flex justify-between">
                                    <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ item.articulo.nombre }}</h2>
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    <div class="flex items-center border border-gray-300 rounded-full overflow-hidden">
                                        <button @click.stop="decrementarCantidad(item)" type="button"
                                        ...
                                        <button @click.stop="incrementarCantidad(item)" type="button"
                                            class="flex items-center justify-center h-6 w-6 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 transition-all">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="ml-auto">
                                        <div v-if="item.articulo.descuento && item.articulo.descuento > 0" class="flex flex-col items-end">
                                            <span class="line-through text-gray-500 dark:text-gray-400 text-xs">{{ item.articulo.precio }} €</span>
                                            <p class="text-gray-900 dark:text-white font-semibold">{{ obtenerPrecioFinal(item.articulo) }} €</p>
                                        </div>
                                        <p v-else class="text-gray-900 dark:text-white font-semibold">{{ item.articulo.precio }} €</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 p-6 bg-gradient-to-r from-purple-100 to-blue-50 dark:from-purple-900/30 dark:to-blue-900/20 rounded-xl shadow-sm border border-purple-200 dark:border-purple-400/10">
                            <div class="flex justify-between items-center mb-4">
                                <p class="text-xl font-bold text-gray-800 dark:text-white">Total:</p>
                                <p class="text-2xl font-extrabold text-purple-700 dark:text-purple-400">{{ precioTotal }} €</p>
                            </div>
                    ...
                    <div v-else class="flex flex-col items-center justify-center h-64 mt-8 p-6 bg-white dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-400 dark:text-gray-500 mb-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                        </svg>
                        <p class="text-center text-lg text-gray-600 dark:text-gray-400 mb-2">Tu carrito está vacío</p>
                        <p class="text-center text-sm text-gray-500 dark:text-gray-500 mb-4">¡Añade algunos productos para comenzar!</p>
                        <Link href="/articulos" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <div class="w-full md:w-3/5 p-4 md:p-6">
                    <div v-if="selectedItem" class="bg-gradient-to-br from-white to-purple-50 dark:from-gray-800/80 dark:to-purple-900/30 rounded-xl p-6 shadow-lg text-gray-800 dark:text-white overflow-hidden border border-gray-200 dark:border-white/5">
                        <div class="flex flex-col md:flex-row gap-6">
                            <!-- Carousel de imágenes del artículo -->
                            <div class="w-full md:w-1/2 relative">
                                <div class="absolute -inset-1 bg-gradient-to-r from-purple-200 to-blue-200 dark:from-purple-600 dark:to-blue-600 rounded-xl blur-sm opacity-20 -z-10"></div>
                                <Carousel
                                    class="carousel-custom rounded-xl overflow-hidden shadow-md bg-white/90 dark:bg-black/40 backdrop-blur-sm"
                                    :items-to-show="1"
                                    :wrap-around="true"
                                    :autoplay="3000"
                                    v-if="selectedItem.articulo.imagenes && selectedItem.articulo.imagenes.length > 0"
                                >
                                    <Slide
                                        v-for="(imagen, index) in selectedItem.articulo.imagenes"
                                        :key="index"
                                    >
                                        <div class="h-64 md:h-80 flex items-center justify-center p-4">
                                            <img
                                                :src="isFullUrl(imagen.ruta) ? imagen.ruta : `/storage/${imagen.ruta}`"
                                                :alt="`Imagen ${index+1} de ${selectedItem.articulo.nombre}`"
                                    ...
                                    <template #addons>
                                        <CarouselNav />
                                        <Pagination v-if="selectedItem.articulo.imagenes.length > 1" />
                                    </template>
                                </Carousel>
                            ...
                            <div class="w-full md:w-1/2 p-4 md:p-6 backdrop-blur-sm bg-white/50 dark:bg-white/5 rounded-xl">
                                <h2 class="text-2xl font-bold mb-2 text-gray-900 dark:text-white">{{ selectedItem.articulo.nombre }}</h2>

                                <!-- Precio con descuento -->
                                <div class="flex items-center gap-2 mb-6">
                                    <div v-if="selectedItem.articulo.descuento && selectedItem.articulo.descuento > 0" class="flex flex-col">
                                        <p class="text-3xl font-bold text-purple-600 dark:text-purple-400">{{ obtenerPrecioFinal(selectedItem.articulo) }} €</p>
                                        <div class="flex items-center gap-2">
                                            <span class="line-through text-gray-500 dark:text-gray-400 text-lg">{{ selectedItem.articulo.precio }} €</span>
                                    ...
                                    <p v-else class="text-3xl font-bold text-purple-600 dark:text-purple-400">{{ selectedItem.articulo.precio }} €</p>
                                    <span class="px-2 py-1 bg-green-50 text-green-600 dark:bg-green-100/20 dark:text-green-400 text-xs font-semibold rounded-full border border-green-200 dark:border-green-400/30">En stock</span>
                                </div>

                                <div class="mb-6 border-t border-b border-gray-200 dark:border-white/10 py-4">
                                    <h3 class="text-lg font-semibold mb-2 flex items-center text-gray-800 dark:text-white">
                                    ...
                                    <div class="text-gray-700 dark:text-gray-300 text-sm leading-relaxed article-content">
                                         {{ selectedItem.articulo.descripcion_short }}
                                    </div>
                                </div>
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="flex items-center border border-purple-300 dark:border-purple-400/30 rounded-lg overflow-hidden bg-white dark:bg-white/5">
                                            <button @click="decrementarCantidad(selectedItem)" type="button"
                                            ...
                                            <span class="w-10 text-center py-1 text-lg font-semibold text-gray-800 dark:text-white">{{ selectedItem.cantidad }}</span>

                                            <button @click="incrementarCantidad(selectedItem)" type="button"
                                                class="flex items-center justify-center h-10 w-10 hover:bg-purple-100 dark:hover:bg-purple-500/20 transition-all text-gray-700 dark:text-white">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                                </svg>
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 mt-6">
                                    <button @click="eliminarDelCarrito(selectedItem)"
                                    ...
                                    <Link :href="`/articulos/${selectedItem.articulo.id}`"
                    ...
                    <div v-else class="h-full flex flex-col items-center justify-center p-10 bg-gradient-to-br from-purple-50 to-blue-50 dark:from-gray-900/50 dark:to-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-300/10">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-purple-300 dark:text-purple-300/40 mb-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                        </svg>
                        <p class="text-lg text-gray-600 dark:text-gray-400 mb-2">Selecciona un artículo para ver sus detalles</p>
                        <p class="text-sm text-gray-500 dark:text-gray-500 text-center max-w-md">Haz clic en cualquier producto de tu carrito para ver más información y opciones</p>
                    </div>
                </div>
            </div>
        </div>
